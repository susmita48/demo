...
export class ConfigService {
  ...
  //offer microservice
  offerserviceUrl = "http://localhost:8074/offer"
  ...
  engageOffer(token: String, offerId: number, empId: number) {
    ...
    return this.http.post<messageResponse>(this.offerserviceUrl + "/engageOffer/"+offerId+"/"+empId, null, options)
  }
  ...
  getAllOffers(token: String){
    ...
    return this.http.get<Offer[]>(this.offerserviceUrl + "/getAllOfferDetails/" , options)
  }
  ...
  //retrive all the offers of the user by emp
  getMyOffers(token: String, id: number) {
    let options = {
      headers: { "Authorization": "Bearer " + token }
    }
    return this.http.get<Offer[]>(this.employeeserviceUrl + "/getallofferbyemp/" + id, options)
  }
